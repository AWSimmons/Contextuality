...
\begin{lem}
Under the extension of a valuation of rank-1 projectors given by the annulus method from Theorem \ref{main} centred on $\ket{\phi}$, a rank-$r$ projector $P_r$ is contextual if and only if $\nicefrac{r}{d}\leq \tr(P_r\ketbra{\phi})<\nicefrac12$.
...
\begin{thm}
In a scenario in which the available measurements are rank-$r$ projectors $\{P_i\}$, peformed on a quantum system of dimension $d$, then at most XXXXXX are given a contextual valuation.
\end{thm}
\begin{proof}
We can apply a proof method identical to that in \ref{main} if we can calculate the proportion of the Hilbert space taken up by the set $\left\{P_r\middle|\nicefrac{r}{d}\leq \tr(P_r\ketbra{\phi})<\nicefrac12\right\}$.
...
\end{equation}
where $I_x(\alpha,\beta)$ is the \emph{regularised incomplete beta function}.
\end{proof}
\section{Future Work}
